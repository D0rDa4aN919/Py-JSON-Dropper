...
class Creator:
    ...
    def json_format(self, binary_file:str) -> None:
        ...
        self.final_json[binary_file.split(".")[0]] = {"body": [seg for seg in encoded[::4]], "fullName": binary_file}
...
def main():
    ...
    user_file = input("Enter the malware file path:")
    if user_file == "":
        print("You didn't enter a file...")
        exit(1)
    files.append(user_file)
    check = input("Add more payloads?[yes/no]")
    ...
    if len(json_file.names) == 1:
        name = json_file.names[0].split('.')[0]
    else:
        name = "payloads_file"
